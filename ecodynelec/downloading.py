...
def download(config, threshold_minutes=15, threshold_size=0.9, is_verbose=False):
    ...
    file_list = {
        k: _get_file_list(
            *dates,
            getattr(config.server, f"_remote{k}Dir"),
    ...
    if config.server.removeUnused:
        _remove_olds(
            config.path.generation, save_list["Generation"]
        )  # Remove unused generation
        _remove_olds(
            config.path.exchanges, save_list["Exchanges"]
    ...
    ### EOF
    if is_verbose:
        print(f"\tDownload from server: {time()-t0:.2f} sec" + " " * 40)
    return
...
def _reach_server(
    ...
    while not is_valid:
        ...
        try:
            ...
            if safety_loop == 0:
                ...
                message += (
                    "Please verify by logging in at https://transparency.entsoe.eu/"
                )
                print(message)
    ...
    if is_verbose:
        print("Create pipe...", end="\r")
    with paramiko.SFTPClient.from_transport(transport) as sftp:
        ### Download all files
        for categ in files:  # Generation then Exchanges
            for i, (remote, local) in enumerate(
                zip(files[categ], savepaths[categ])
            ):  # For each file
                if not _should_download(
                    sftp, remote, local, threshold_minutes, threshold_size
                ):
                    continue

                callback_fct = None
                if is_verbose:
                    callback_fct = partial(
                        _progressBar, info=f"{categ} {i+1}/{len(files[categ])}"
                    )

                sftp.get(remotepath=remote, localpath=local, callback=callback_fct)
...
def _set_time(start, end):
    ...
    if None not in [start, end]:  # Regular method
        ...
        all_months = pd.period_range(
            start=start, end=datetime.utcnow(), freq="M", periods=2
        )
    elif end is None:  # Start but no end: until now
        all_months = pd.period_range(
            start=start, end=datetime.utcnow(), freq="M", periods=None
        )
...
def _get_file_list(years, months, path, rootName):
    ...
    with exact names.
    """
    return [f"{path}{y}_{m:02d}_{rootName}" for y, m in zip(years, months)]
...
def _should_download(sftp, remote, local, threshold_minutes=15, threshold_size=0.9):
    ...
    )  # Server is in UTC. Local files are in local time.
    is_newer = (remote_mtime - (local_mtime + delta_utc)) > timedelta(
...
def _remove_olds(path, local_list):
    ...
    remove_list = [
        ...
        if ((f not in local_list) & (os.path.isfile(path + f)))
    ]
...
def _progressBar(transferred, toBeTransferred, info):
    ...
    print(
        f"[{info}] Transferred: {transferred/1024**2:.1f} MB\tOut of: {toBeTransferred/1024**2:.1f}"
        + " " * 10,
        end="\r",
    )
